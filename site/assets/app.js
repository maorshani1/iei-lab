...
(() => {
  ...
  $$('[data-download]').forEach(b=>b.addEventListener('click',()=>{switch(b.dataset.download){case'hope-csv':{const h=data.hope,s=h.studies[Number($('#hope-study').value)];download('seri-hope-published-correlations.csv',csv([['study','outcome','hope_for_peace_r','hope_for_victory_r','source','source_location','data_kind','sample_note'],...h.outcomes.map((o,i)=>[s.label,o,s.peace[i],s.victory[i],h.source,h.sourceLocation,h.kind,s.sample])]),'text/csv;charset=utf-8');break;}case'profile-csv':{const p=data.profiles;download('seri-germany-published-profile-summaries.csv',csv([['profile','profile_n','outcome','mean','standard_deviation','source','source_location','data_kind'],...p.outcomes.flatMap(o=>p.groups.map((g,i)=>[g.name,g.n,o.name,o.means[i],o.sd[i],p.source,p.sourceLocation,'Published aggregate; not individual observations']))]),'text/csv;charset=utf-8');break;}case'hope-svg':exportSVG('#hope-chart','seri-hope-correlation-chart.svg');break;case'profile-svg':exportSVG('#profile-chart','seri-germany-profile-chart.svg');break;}}));
